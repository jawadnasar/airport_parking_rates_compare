...
window.addEventListener('load', function () {
   ...
   (document.head || document.documentElement).appendChild(s);
    ...
    /* Header Start */
    const header_button_div = create_header_for_new_form_reg();
        ...
        for (var key in myArray) {
           ...
           if (changeid !=undefined){
            ...
            if (changeid.tagName=='SELECT' && changeid.options.length>0){
                ...
                if(idtochange=='province_id'){
                   // $("#select2-province_id-container").trigger('change');
                   // setTimeout('', 3000);
                    
                }
                if(idtochange=='ptn'){
                   // $("#ptn").select2().trigger('change');
                    setTimeout('', 3000);
...
function create_header_for_new_form_reg() {
